...
#ATTRIBUTE:FeatureScope;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Farm,Site,Web,WebApplication
$FeatureScope = Get-AnsibleParam -obj $params -name FeatureScope -failifempty $True -resultobj $result
...
If ($FeatureScope)
{
    If (('Farm','Site','Web','WebApplication') -contains $FeatureScope ) {
    }
    Else
    {
        Fail-Json $result "Option FeatureScope has invalid value $FeatureScope. Valid values are 'Farm','Site','Web','WebApplication'"
    }
}
