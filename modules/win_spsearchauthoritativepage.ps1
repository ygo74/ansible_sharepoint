...
#ATTRIBUTE:Action;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:Authoratative,Demoted
...
#ATTRIBUTE:Level;MANDATORY:False;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$Level = Get-AnsibleParam -obj $params -name Level -failifempty $False -resultobj $result
...
If ($Action)
{
    If (('Authoratative','Demoted') -contains $Action ) {
    }
    Else
    {
        Fail-Json $result "Option Action has invalid value $Action. Valid values are 'Authoratative','Demoted'"
    }
}
...
$DscResourceName = "SPSearchAuthoritativePage"
