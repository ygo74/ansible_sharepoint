...
#ATTRIBUTE:DnsName;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:
$DnsName = Get-AnsibleParam -obj $params -name DnsName -failifempty $True -resultobj $result
#ATTRIBUTE:Zone;MANDATORY:True;DEFAULTVALUE:;DESCRIPTION:;CHOICES:External-HTTP,External-HTTPS,Internal-HTTP,Internal-HTTPS
...
If ($Zone)
{
    If (('External-HTTP','External-HTTPS','Internal-HTTP','Internal-HTTPS') -contains $Zone ) {
    }
    Else
    {
        Fail-Json $result "Option Zone has invalid value $Zone. Valid values are 'External-HTTP','External-HTTPS','Internal-HTTP','Internal-HTTPS'"
    }
}
...
$DscResourceName = "SPOfficeOnlineServerBinding"
